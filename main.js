...
async function checkMembership(userId) {
  try {
    ...
    const isInChannel2 = await bot.telegram.getChatMember(CHANNEL_ID_2, userId);
    return (
      isInGroup.status !== 'left' &&
      isInChannel.status !== 'left' &&
      isInChannel2.status !== 'left'
    );
  } catch (err) {
...
async function verifyUser(ctx, next) {
    ...
    if (!isMember) {
        return ctx.replyWithPhoto(global.pp, {
            caption: "❌ *Access Denied!*\n\nYou must join, subscribe and follow all after /start again.",
            parse_mode: "Markdown",
            reply_markup: {
                inline_keyboard: [
                    [{ text: "🍏 WhatsApp 🍏", url: WHATSAPP_LINK }],
                    [{ text: "🍎️ YouTube 🍎️", url: YOUTUBE_LINK }],
                    [{ text: "🫐 Telegram Group 🫐", url: GROUP_LINK }],
                    [{ text: "🍇 Telegram Channel 🍇", url: CHANNEL_INVITE_LINK }],
                    [{ text: "💀 𝐁𝐀𝐂𝐊𝐔𝐏 ♦️", url: CHANNEL_INVITE_LINK_2 }],
                    [{ text: "⚠️ Verify ✔️", callback_data: "check_membership" }]
                ]
            }
        });
    } else {
...
async function startXeony() {
    ...
    try {
        ...
        const username = user.username ? user.username : "RISHIHEARTMAKER";
    ...
    try {
        ...
        if (profilePhotos.photos.length > 0) {
            ...
            await XeonBotInc.replyWithPhoto(ownerPhoto, {
                ...
                reply_markup: {
                    inline_keyboard: [
                        [{ text: '𝑶𝑾𝑵𝑬𝑹 ヅ', url: "https://t.me/RISHIHEARTMAKER" }, { text: '𖤍 𝑪𝑯𝑨𝑵𝑵𝑬𝑳 𖤍', url: "https://t.me/TEAM56RJ" }, { text: '♦️ 𝑮𝑹𝑶𝑼𝑷 ♦️', url: "https://t.me/rishiheartmaker1" }]
                    ]
                }
            });
        } else {
        ...
        await XeonBotInc.reply(lang.first_chat(BOT_NAME, user.full_name), {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: '𝑶𝑾𝑵𝑬𝑹 ヅ', url: "https://t.me/RISHIHEARTMAKER" }, { text: '♦️ 𝑮𝑹𝑶𝑼𝑷 ♦️', url: "https://t.me/rishiheartmaker1" }]
                ]
            }
        });
    }
...
*Hold on it to copy the ID.*`;
    ...
    // Execute RishiXtele.js only if the user is verified
    require("./RishiXtele")(XeonBotInc, bot);
    ...
    bot.telegram.getMe().then((getme) => {
        console.table({
            ...
            "Author": "https://t.me/RISHIHEARTMAKER"
        })
    })
    ...
    if (!Array.isArray(adminIDs) || !adminIDs.includes(userID)) {
        return ctx.replyWithMarkdownV2(
            ...
            `📩 *Send proof to the owner @DGXeon*`, 
            {
                reply_markup: {
                    inline_keyboard: [
                        [{ text: "📢 Telegram Channel", url: "https://t.me/rishiheartmaker1" }],
